...
- `IPlcDriver` 厂商隔离接口。
- `SiemensPlcDriver`：S7-1500 连接、状态读取、命令脉冲、协议版本检查、可配置命令应答和双向心跳。
- `PlcPollingLoop`：50 ms 周期、断线清理、2 秒重连、运行统计。
- `SimulatedPlcDriver`：无需硬件即可注入读取失败、命令写入失败、处理延时和业务拒绝。
- PLC 地址通过 `appsettings.json` 配置，并与 DB100/DB101 点表 V1 草案对齐。
...
| PLC-006 | 协议版本错误 | PLC/PC 版本不一致 | 连接失败并给出双方版本 |
| PLC-007 | 命令业务拒绝 | PLC 返回非零结果码 | 上位机报告拒绝，不误触发网络重连 |
| PLC-008 | 命令应答丢失 | 不更新 AckSequence | 超时后进入通信故障恢复 |
| PLC-009 | PLC 心跳冻结 | 保持 PlcHeartbeat 不变化 | 超过阈值后识别为故障并重连 |
| PLC-010 | 8 小时耐久 | 连续运行 | Crash=0，统计完整 |
| PLC-011 | 24 小时 Gate | 连续运行 | Crash=0，无资源持续增长 |
...
- PLC 已实现 `CommandSequence/AckSequence/Busy/Error/Status` 和心跳递增逻辑后，才把对应功能开关设为 `true`。
