...
| 02 | PLC Driver | 进行中 | S7.NetPlus、批量变量读取、命令脉冲、重连、配置校验、模拟器 | 点表 V1；真实 S7-1511 8h/24h；断线和重启恢复 |
...
当前必须继续完成 02 PLC Driver。代码侧已完成批量读取和配置校验，下一项是冻结 DB100/DB101 点表并连接真实 S7-1511。真实 PLC Gate 未完成前，不把 03 Camera Driver 标记为进行中。

## 02 PLC Driver 下一批任务

1. 根据 TIA Portal V18 工程冻结 DB100_HMI、DB101_Machine 的符号、偏移和数据类型。
