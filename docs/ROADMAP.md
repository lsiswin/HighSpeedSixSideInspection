...
| 02 | PLC Driver | 进行中 | S7.NetPlus、V1 点表、协议版本、命令应答/心跳；TIA V18 通信 FB、实例 DB、OB1 调用已 0/0 编译 | PLCSIM/真机读写与异常恢复；真实 S7-1511 8h/24h；冻结 V1 |
...
当前必须继续完成 02 PLC Driver。DB100/DB101、PLC 侧握手/心跳 FB、实例 DB 和 OB1 调用已经导入 TIA V18，并通过 PLC_1 完整编译（0 错误、0 警告）。下一项是在 PLCSIM Advanced 或真实 S7-1511 上验证读写、命令完成应答、心跳冻结、断线和重启恢复。真实 PLC Gate 未完成前，不把 03 Camera Driver 标记为进行中。

## 02 PLC Driver 下一批任务

1. 确认 S7-1511 固件、IP、Rack、Slot、PUT/GET、优化访问及通讯权限。
2. 在 PLCSIM Advanced 或隔离测试 PLC 下载经评审的离线工程，不连接生产执行机构。
3. 在安全测试模式验证 Read/Write、命令脉冲、流水号去重、RecipeChange 和完成后应答。
4. 将 `Main` 的 `CommandDone`/`CommandStatus` 从安全占位值接入后续 PLC 机器状态机。
5. 执行拔网线、PLC 重启、错误地址、应答超时、心跳冻结和恢复测试。
6. 执行 8 小时 PoC，达标后再执行 24 小时 Gate，并冻结 V1。
